    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (db_con != null) {
             ...
             db_st = db_con.prepareStatement("select*from profiles where name like ?"); //『profiles』テーブルの『name』に『?』が含まれる
             db_st.setString(1,"%茂%"); //１番目の『?』に対して、『茂』を設定する処理＝『profiles』テーブルの『name』に『茂』が含まれる
             db_data = db_st.executeQuery();

            while (db_data.next()) {    //『profiles』の中に『茂』を含む情報があればそのレコードを表示する
